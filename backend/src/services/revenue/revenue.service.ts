...
export interface CreatorMetrics {
    ...
    growth: {
        subscribers: number; // % change from last month
        revenue: number; // % change from last month
    };
}

/**
 * Monthly earnings interface
 */
...
export class RevenueService {
    ...
    async getCreatorMetrics(creatorId: string): Promise<CreatorMetrics> {
        try {
            ...
            logger.error('Error calculating creator metrics:', error);
            throw error;
        }
    }
     ...
     */
    async getEarningsHistory(creatorId: string, months: number = 12): Promise<MonthlyEarnings[]> {
        try {
            ...
            for (let i = 0; i < months; i++) {
                ...
                result.push({
                    ...
                    subscribers: newSubs,
                    payoutStatus: payout ? (payout.status as any) : 'none',
                });
            }
     ...
     */
    async getPendingPayout(creatorId: string): Promise<number> {
        try {
            ...
            logger.error('Error getting pending payout:', error);
            throw error;
        }
    }
     ...
     * Request a payout
     */
    async requestPayout(creatorId: string): Promise<any> {
        try {
            ...
            logger.info(`Payout ${payout.id} initiated for creator ${creatorId}: $${pendingAmount}`);
